// Test the fixed reports API approach
async function testFixedReportsApi() {
  ...
  try {
    // Define fonts properly for server-side generation
    const fonts = {
      Roboto: {
    ...
    const docDefinition = {
      content: [
        ...
        { text: `Generated at: ${new Date().toLocaleString('id-ID')}`, style: 'subheader' },
        { text: '\n' },
        { text: 'This is a test PDF generated using the fixed approach.' }
      ],
      styles: {
        header: {
          fontSize: 18,
    ...
    pdfDoc.on('end', () => {
      ...
      console.log('PDF generated successfully using fixed approach. Size:', result.length, 'bytes');
    });
